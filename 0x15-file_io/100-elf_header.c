...
void print_magic(unsigned char *e_ident)
{
	...
	for (space = 0; space < EI_NIDENT; space++)
	{
		printf("%02x", e_ident[space]);

		if (space == EI_NIDENT - 1)
			printf("\n");
		else
			printf(" ");
	}
 ...
 * @e_ident: The pointer to the array which contains the ELF ABI version.
 */
void print_abi(unsigned char *e_ident)
{
...
int main(int __attribute__((__unused__)) argc, char *argv[])
{
	...
	i = read(o, header, sizeof(Elf64_Ehdr));
	if (i == -1)
	{
		free(header);
		close_elf(o);
